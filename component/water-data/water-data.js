...
Component({
    ...
    methods: {
      dis_WaterLevel(waterLevel) {
        let r = '--';
        if (waterLevel >= 0) r = waterLevel.toFixed(4);
        this.setData({
          v_waterLevel: r
        });
      },
      ...
      dis_RadarError(radarErrorCode) {
        ...
        // 雷达自身报警
        if (radarErrorCode) {
          if (radarErrorCode & 0x0001) r += "丢波;"
          if (radarErrorCode & 0x0002) r += "TR通信异常;"
          if (radarErrorCode & 0x0004) r += "无阈值数据;"
          if (radarErrorCode & 0x0008) r += "电流异常;"
          if (radarErrorCode & 0x0010) r += "电流手动;"
          if (radarErrorCode & 0x0020) r += "LCD异常;"
          if (radarErrorCode & 0x0040) r += "TR连接异常;"
          if (radarErrorCode & 0x0080) r += "时钟异常;"
          if (radarErrorCode & 0x0100) r += "处理器异常;"
          if (radarErrorCode & 0x0200) r += "温度传感器异常;"
          if (radarErrorCode & 0x0400) r += "采集异常;"
        }
        // 判断通讯错误
        if (this.data.commError) {
          if (this.data.commError == 1) r += '通讯异常;'
        }
        this.setData({
          v_error: r
        });
