...
import {
  f_setMenu,
  f_setList
} from '../../utils/f_setmenu'
import {
  translate_radarcom,
} from '../../utils/analysisradardata'
import {
  f_cmdParam
} from '../../utils/f_cmdparam'
import {
  buf2int,
  buf2float,
  buf2str
...
Page({
  ...
  onLoad(options) {
    ...
    app.globalData.ble_device.device_type = this.data._lang['流速计'];
    //绑定 接收信息的回调函数
  ...
  initWaveQueue() {
    ...
    this.data.sendWaveQueueObject.totalCommandCount += 3;
  ...
  startReadEcho() {
    ...
    console.log(this.data.sendWaveQueueObject.commandSendQueue[0]);
    this.setData({
      readWaveFlag: true,
    });
  },
